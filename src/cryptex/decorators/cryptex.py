    ...
    def decorator(func: F) -> F:
        ...
        if framework:
            ...
            if context not in ["fastmcp", "fastapi"]:
                raise ValueError(f"Unsupported framework '{framework}'. Supported: 'fastmcp', 'fastapi'")
        else:
...
def _detect_framework_context(frame: Optional[FrameType], func: Callable) -> str:
    """
    ...
    try:
        ...
        if hasattr(func, '__annotations__'):
            ...
            for param_name, annotation in annotations.items():
                ...
                if any(fastapi_indicator in annotation_str.lower() for fastapi_indicator in 
                       ['header', 'query', 'path', 'body', 'depends', 'fastapi']):
                    return "fastapi"
        
        # Third check: Function metadata
        if hasattr(func, '__dict__'):
            func_dict = func.__dict__
            if any(key.startswith('_fastapi') or 'route' in key.lower() for key in func_dict.keys()):
                return "fastapi"
...
def _get_calling_module() -> Optional[str]:
            ...
            for var_name, var_value in frame_locals.items():
                if var_value is not None:
                    try:
                        ...
                        if (any(mcp_indicator in var_name.lower() for mcp_indicator in ['mcp', 'server', 'tool']) and
                            ('mcp' in class_name or 'server' in class_name)):
                            return "fastmcp"
                        
                        # Check for FastAPI indicators
                        if (any(fastapi_indicator in var_name.lower() for fastapi_indicator in ['app', 'fastapi', 'router']) and
                            ('fastapi' in class_name or 'router' in class_name)):
                            return "fastapi"
...
def cryptex_file(config_path: Optional[str] = None) -> Callable[[F], F]:
...
def cryptex_api(config_path: Optional[str] = None) -> Callable[[F], F]:
...
def cryptex_database(config_path: Optional[str] = None) -> Callable[[F], F]:
...
def cryptex_auth(config_path: Optional[str] = None) -> Callable[[F], F]:
