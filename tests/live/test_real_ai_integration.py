...
class TestLiveAIIntegration:
    ...
    async def test_live_openai_temporal_isolation(self):
        ...
        if received_key == '{{OPENAI_API_KEY}}':
            ...
            assert False, "Real API key leaked to AI service"
        else:
            print(f"❓ UNEXPECTED: AI received: {received_key}")
            print("Result:", result)
            assert False, f"Unexpected API key received: {received_key}"
    ...
    def test_dogfooding_protection(self):
        ...
        if '{{OPENAI_API_KEY}}' in prompt and '{{FILE_PATH}}' in prompt:
            ...
            assert False, "Secrets not properly sanitized in AI prompt"

    def test_live_test_instructions(self):
        """Provide instructions for running live tests."""
        print(f"\n📋 To run live tests:")
        print(f"   1. Set environment variables:")
        print(f"      export OPENAI_API_KEY='your-real-openai-key'")
        print(f"      export RUN_LIVE_TESTS=1")
        print(f"   2. Run with pytest:")
        print(f"      pytest tests/live/test_real_ai_integration.py -v -s")
        print(f"   3. This will make real API calls and cost money!")
        print(f"   4. Only run this when you want to validate temporal isolation works with real AI services")
